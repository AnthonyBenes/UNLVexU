/**
 * This program build together with tasks.c file and autonomous1.c file
 * @author Pat Collins and Ryan
 * Modified by Dat Nguyen
 * Date feb 21st , 2018
 * version: 5.1
 * modified:
 * user will be able to run "2" tasks at once
 *TODO: create a autonomous2.c for autonomouse stage during match.
 */
...
task usercontrol()
{
	...
	while (true)
	{
		...
		/**
		 *change orientation of the bot.
		 *User can choose front as head or back as head.
		 */
		...
		// low lift position handling
		if(lowLift == true && SensorValue[lowerMobileGoalLimitUP] == 0){
		...
		if(lowLift == false && SensorValue[leftLowMGDown] == 0){
			motor[liftMobileLowerLeft] = -30;
		}else{motor[liftMobileLowerLeft] = 0;}

		if(lowLift == false && SensorValue[rightLowMGDown] == 0){
			motor[liftMobileLowerRight] = -30;
		}else{motor[liftMobileLowerRight] = 0;}
/***************************END******Low Lift***************************/



/*********************HIGH LIFT***********************************/
		int frontLiftPosition = 1 ;// initialize position for front lift
		...
		// front lift position handling
		// left lift
		if(frontLiftPosition = 1 && SensorValue[liftAngleLeft] > 1270){
			motor[liftMobileHigherLeft] = 127;
		}else{motor[liftMobileHigherLeft] = 0;}

		if(frontLiftPosition = 2 && SensorValue[liftAngleLeft] > 1940){
			motor[liftMobileHigherLeft] = -127;
		}else{motor[liftMobileHigherLeft] = 0;}
		...
		if(frontLiftPosition = 3 && SensorValue[liftAngleLeft] < 3030){
			motor[liftMobileHigherLeft] = 127;
		}else{motor[liftMobileHigherLeft] = 0;}
